...
```yaml
schedule:
  # 每小时运行一次，覆盖洛杉矶时间 6am-11pm
  - cron: '17 13-23 * * *'  # UTC 13:17-23:17
  - cron: '17 0-6 * * *'    # UTC 00:17-06:17
```

**时区换算（PDT 夏令时，UTC-7）：**
- UTC 13:17-23:17 = 洛杉矶 06:17-16:17
- UTC 00:17-06:17 = 洛杉矶 17:17-23:17

**为什么是 :17 分钟而不是整点？**
GitHub Actions 的 cron 调度是 best-effort：UTC 整点（`0 * * * *`）是全平台 cron 任务最集中的时刻，runner 池容易拥堵，导致任务**被延迟数十分钟甚至直接跳过**。把分钟错开到 `:17` 这种非常见值，可以显著降低被影响的概率。即便如此，仍可能偶尔有几次运行被延迟或跳过，这是 GitHub Actions 调度本身的限制。

如需调整可修改 `.github/workflows/crawl.yml`。
...
- **前端加载时间**：<1 秒（纯静态）
- **数据新鲜度**：洛杉矶时间 6am-11pm 每小时一次（共 18 次/天）
- **存储空间**：<1MB（JSON数据极小）
...
3. **无用户账户**：不支持个人偏好保存
4. **速率限制**：如果频率过高可能被AMC网站限制（当前每小时一次，已有缓存策略）
